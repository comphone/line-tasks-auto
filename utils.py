def dateutil_parse(date_string):
    """Wrapper function for dateutil parse to use in templates"""
    return date_parse(date_string)import os
import re
import json
import pytz
import zipfile
...
def get_notes_parts(notes):
    ...
    for block in report_blocks:
        try:
            history.append(json.loads(block))
    ...
    if feedback_blocks:
        try:
            feedback = json.loads(feedback_blocks[0])
        except json.JSONDecodeError:
            feedback = {}
...
def parse_tech_report_from_notes(notes):
    """Extract tech reports from notes - returns (history, base_notes_text)"""
    if not notes:
        return [], notes
    ...
    # Remove tech reports from notes to get base notes
    base_notes_text = re.sub(report_pattern, '', notes, flags=re.DOTALL).strip()
    
    # Also remove customer feedback if present
    feedback_pattern = r"--- CUSTOMER_FEEDBACK_START ---\s*\n(.*?)\n--- CUSTOMER_FEEDBACK_END ---"
    base_notes_text = re.sub(feedback_pattern, '', base_notes_text, flags=re.DOTALL).strip()
...
def parse_customer_feedback_from_notes(notes):
    """Extract customer feedback from notes"""
    if not notes:
        return {}
    ...
    if feedback_blocks:
        try:
            return json.loads(feedback_blocks[0])
        except json.JSONDecodeError:
            return {}
    return {}
...
def create_backup_zip():
    """Creates a comprehensive backup zip file containing all system data"""
    try:
        memory_file = BytesIO()
        with zipfile.ZipFile(memory_file, 'w') as zf:
            # Add settings
            settings = get_app_settings()
            zf.writestr('settings_backup.json', json.dumps(settings, ensure_ascii=False, indent=2))
            
            # Add all tasks
            tasks = gs.get_google_tasks_for_report(show_completed=True) or []
            zf.writestr('tasks_backup.json', json.dumps(tasks, ensure_ascii=False, indent=2))
            ...
            for root, dirs, files in os.walk('.'):
                ...
                for file in files:
                    if file.endswith(('.py', '.html', '.txt', '.yaml', '.yml', '.json', '.css', '.js', '.env.example')):
                        file_path = os.path.join(root, file)
                        arc_name = os.path.relpath(file_path, '.')
                        try:
                            zf.write(file_path, arc_name)
                        except Exception as e:
                            print(f"Could not add {file_path} to backup: {e}")
        
        memory_file.seek(0)
        filename = f"comphone_backup_{datetime.now(THAILAND_TZ).strftime('%Y%m%d_%H%M%S')}.zip"
