...
def summary():
    ...
    for task in tasks_raw:
        if task.get('status') == 'completed' and task.get('completed'):
            try:
                key = datetime.datetime.fromisoformat(task['completed'].replace('Z', '+00:00')).astimezone(THAILAND_TZ).strftime('%Y-%m')
                if key in monthly_completed: monthly_completed[key] += 1
            except (ValueError, TypeError): continue
    sorted_months = sorted(monthly_completed.keys())
    chart_data = {'labels': [datetime.datetime.strptime(m, '%Y-%m').strftime('%b %y') for m in sorted_months], 'values': [monthly_completed[m] for m in sorted_months]}
...
def edit_task(task_id):
    ...
    if request.method == 'POST':
        ...
        notes_lines.extend([f"ลูกค้า: {str(request.form.get('customer_name', '')).strip()}", f"เบอร์โทรศัพท์: {str(request.form.get('customer_phone', '')).strip()}", f"ที่อยู่: {str(request.form.get('address', '')).strip()}"])
...
def technician_report():
    ...
    return render_template('technician_report.html', report_data=report_data, selected_year=year, selected_month=month, years=list(range(now.year - 5, now.year + 2)), months=months, technician_list=technician_list)
