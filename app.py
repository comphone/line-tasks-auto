        ...
        for task in follow_up_tasks:
            ...
            if customer_line_user_id_for_task:
                ...
                admin_message_text = (
                    ...
                    f"รายละเอียดงาน: {task.get('title', '-').splitlines()[0] if task.get('title') else '-'}\n"
                    f"ลิงก์งาน: {url_for('task_details', task_id=current_app.config.get('task_id_for_url_for_fallback', 'some_default_task_id'), _external=True)}"
                )
                ...
                try:
                    if admin_group_id:
                        ...
                        print(f"INFO: Sent group follow-up messages for task {current_app.config.get('task_id_for_url_for_fallback', 'some_default_task_id')} to admin group.", file=sys.stderr)
                    if technician_group_id and technician_group_id != admin_group_id:
                        current_app.line_bot_api.push_message(technician_group_id, messages_to_send_group)
                        print(f"INFO: Sent group follow-up messages for task {current_app.config.get('task_id_for_url_for_fallback', 'some_default_task_id')} to technician group.", file=sys.stderr)
                except Exception as e:
                    print(f"ERROR: Failed to send group follow-up LINE messages for task {current_app.config.get('task_id_for_url_for_fallback', 'some_default_task_id')}: {e}", file=sys.stderr)
