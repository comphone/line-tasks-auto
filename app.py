...
def run_scheduler():
    ...
    # --- เพิ่ม Scheduled Customer Follow-up Job (ฟีเจอร์ใหม่) ---
    followup_hour = settings.get('report_times', {}).get('customer_followup_hour_thai', 9)
    follow_up_job_id = 'daily_customer_follow_up'
    if not scheduler.get_job(follow_up_job_id):
        scheduler.add_job(
            scheduled_customer_follow_up_job,
            CronTrigger(hour=followup_hour, minute=0, timezone=THAILAND_TZ),
            id=follow_up_job_id
        )
...
# ===== [ฟีเจอร์ใหม่ Customer Feedback/Follow-up] =====
