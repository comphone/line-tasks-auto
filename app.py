...
def form_page():
    ...
    # Pass taskDetailSnippets to the form.html template
...
def api_upload_attachment():
    ...
    media_body = gs.MediaIoBaseUpload(file_to_upload, mimetype=mime_type, resumable=True) # Use gs.MediaIoBaseUpload
    drive_file = gs._perform_drive_upload(media_body, filename, mime_type, final_upload_folder_id) # Use gs._perform_drive_upload
    ...
    if drive_service:
        for att in original_attachments:
            if att['id'] in attachments_to_keep_ids:
                ...
                try:
                    gs._execute_google_api_call_with_retry(drive_service.files().delete, fileId=att['id']) # Use gs._execute_google_api_call_with_retry
                    app.logger.info(f"Deleted attachment {att['id']} from Drive.")
                except gs.HttpError as e: # Use gs.HttpError
    ...
    if new_files:
        ...
        if final_upload_folder_id:
            for file in new_files:
                if file and utils.allowed_file(file.filename): # Use utils.allowed_file
                    file.seek(0, os.SEEK_END)
                    file_length = file.tell()
                    file.seek(0)
