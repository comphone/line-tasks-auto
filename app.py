...
app.LIFF_ID_FORM = os.environ.get('LIFF_ID_FORM') # Make LIFF ID available in app context
...
# --- Utility function required by other modules ---
    ...
    try:
        ...
        with zipfile.ZipFile(memory_file, 'w', zipfile.ZIP_DEFLATED) as zf:
            ...
            for folder, _, files in os.walk(project_root):
                for file in files:
                    if file.endswith(('.py', '.html', '.css', '.js', '.json', 'Procfile', 'requirements.txt', '.yaml')) and '.venv' not in folder:
...
def oauth2callback():
    ...
    flow = Flow.from_client_config(client_config, scopes=gs.SCOPES, state=state, redirect_uri=url_for('authorize', _external=True, _scheme='https'))
