...
def configure_app(app_instance):
                ...
                if new_attachments_uploaded:
                    for file in files:
                        if file and allowed_file(file.filename):
                            ...
                            temp_filepath = os.path.join(_app.config['UPLOAD_FOLDER'], filename) 
                            try:
    ...
    @app_instance.route("/callback", methods=['POST']) # This remains the Flask route decorator
    def callback():
    ...
    # Define other Flask routes that are LIFF-related (must be inside here too)
...
# This structure ensures `app` is defined before any decorators attempt to use it.
configure_app(app) # Call the configuration function with the global app instance
