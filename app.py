        ...
        for task in follow_up_tasks:
            ...
            if customer_line_user_id_for_task:
                ...
                admin_message_text = (
                    ...
                    f"รายละเอียดงาน: {task.get('title', '-').splitlines()[0] if task.get('title') else '-'}\n"
                    f"ลิงก์งาน: {url_for('task_details', task_id=current_app.config.get('task_id_for_url_for_fallback', 'some_default_task_id'), _external=True)}" # Fallback for url_for context.
                )
