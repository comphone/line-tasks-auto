...
def task_details(task_id):
    ...
    p_task['tech_reports_history'], _ = utils.parse_tech_report_from_notes(p_task.get('notes', ''))
    p_task['customer'] = utils.parse_customer_info_from_notes(p_task.get('notes', ''))
    
    # Pre-format report dates before rendering
    if p_task.get('tech_reports_history'):
        for report in p_task['tech_reports_history']:
            if report.get('summary_date'):
                try:
                    ...
                    report['summary_date_formatted'] = report['summary_date'] # Fallback
            else:
                report['summary_date_formatted'] = 'N/A'

    all_attachments = []
    for report_item in p_task.get('tech_reports_history', []):
...
def settings_page():
    ...
    app.logger.info(f"Loading settings page. Technician list: {current_settings.get('technician_list')}")
